...
test( 'H1 がない文書を拒否する', () => {
	const markdown = validMarkdown.replace( '# Test Architecture\n\n', '' );

	assert.throws(
		() => validateArchitectureMarkdownStructure( markdown ),
		/exactly one level-1 heading is required; found 0/u
	);
} );

test( '複数の H1 がある文書を拒否する', () => {
	const markdown = validMarkdown.replace(
		'# Test Architecture',
	...
	assert.throws(
		() => validateArchitectureMarkdownStructure( markdown ),
		/exactly one level-1 heading is required; found 2/u
	);
} );
